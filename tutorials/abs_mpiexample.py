...
def main():
    ...
    ABSBIN = 10  # angular mode bin number
    ...
    raw_b_signal = None
    if not mpirank:
        raw_t_noise = np.empty([mpisize*NSAMP, len(ellist), NFREQ, NFREQ], dtype=raw_t[1].dtype)
        raw_t_signal = np.empty([mpisize*NSAMP, len(ellist), NFREQ, NFREQ], dtype=raw_t[1].dtype)
        raw_e_noise = np.empty([mpisize*NSAMP, len(ellist), NFREQ, NFREQ], dtype=raw_eb[1].dtype)
        raw_e_signal = np.empty([mpisize*NSAMP, len(ellist), NFREQ, NFREQ], dtype=raw_eb[1].dtype)
        raw_b_noise = np.empty([mpisize*NSAMP, len(ellist), NFREQ, NFREQ], dtype=raw_eb[1].dtype)
        raw_b_signal = np.empty([mpisize*NSAMP, len(ellist), NFREQ, NFREQ], dtype=raw_eb[1].dtype)
    ...
    if not mpirank:
        ...
        for s in range(mpisize*NSAMP):
            ...
            spt_t = ap.abssep(raw_t_signal[s],t_noise_mean,t_noise_std_diag,modes=ellist,bins=safe_absbin,shift=SHIFT_T,threshold=CUT_T)
            rslt_t = spt_t()
            if (s==0):
                rslt_ell = rslt_t[0]
            rslt_Dt += rslt_t[1]
        ...
        for s in range(mpisize*NSAMP):
            # send PS to ABS method
            safe_absbin = min(len(ellist), ABSBIN)
            spt_e = ap.abssep(raw_e_signal[s],e_noise_mean,e_noise_std_diag,modes=ellist,bins=safe_absbin,shift=SHIFT_EB,threshold=CUT_EB)
            spt_b = ap.abssep(raw_b_signal[s],b_noise_mean,b_noise_std_diag,modes=ellist,bins=safe_absbin,shift=SHIFT_EB,threshold=CUT_EB)
            rslt_e = spt_e()
            rslt_b = spt_b()
            if (s==0):
                rslt_ell = rslt_e[0]
        ...
        output = np.zeros((7,len(rslt_ell)))
        output[0] = rslt_ell
        ...
        est = ap.pstimator(nside=NSIDE,mask=maskmap.reshape(1,-1), aposcale=5.0, psbin=20)
        auto_cmb_t = est.auto_t(cmbmap[0].reshape(1,-1),fwhms=FWHMS[0])
        auto_cmb_eb = est.auto_eb(cmbmap[1:].reshape(2,-1),fwhms=FWHMS[0])
        # add extra binning
        binned_ell = ap.binell(list(auto_cmb_t[0]), ABSBIN)
        binned_cmb_t = ap.binaps(auto_cmb_t[1].reshape(-1,1), list(auto_cmb_t[0]), ABSBIN)
        binned_cmb_e = ap.binaps(auto_cmb_eb[1].reshape(-1,1), list(auto_cmb_t[0]), ABSBIN)
        binned_cmb_b = ap.binaps(auto_cmb_eb[2].reshape(-1,1), list(auto_cmb_t[0]), ABSBIN)
        output[0] = binned_ell
        output[1] = list(binned_cmb_t)
        output[2] = list(binned_cmb_e)
        output[3] = list(binned_cmb_b)
